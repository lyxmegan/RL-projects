...
if __name__=="__main__":
    ...
    inps = [torch.from_numpy(np.array([0, 1, 2])).long(),
            torch.from_numpy(np.array([0, 1, 3])).long(),
            torch.from_numpy(np.array([0, 4, 2])).long(),
            torch.from_numpy(np.array([1, 1, 2])).long(),
            torch.from_numpy(np.array([0, 0, 0, 0, 1, 2])).long()]
    print("Standard LSTM with random parameters")
    check_distance(rnn_module, inps)
    print("======")
    print("Forget gate bias set high")
    set_forget_bias(rnn_module, 10.)
    check_distance(rnn_module, inps)
    print("======")
    print("Forget gate bias set low")
    set_forget_bias(rnn_module, -10.)
    check_distance(rnn_module, inps)
    print("======")
    print("Input gate bias set high")
    set_forget_bias(rnn_module, 0.)
    set_input_bias(rnn_module, 10.)
    check_distance(rnn_module, inps)
    print("======")
    print("Input gate bias set low")
    set_forget_bias(rnn_module, 0.)
    set_input_bias(rnn_module, -10.)
    check_distance(rnn_module, inps)
